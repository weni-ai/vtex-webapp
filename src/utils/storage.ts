...
class ModuleStorage {
   ...
   */
  setItem(key: string, value: any, options = { useSession: false }) {
   ...
   */
  getItem(key: string, defaultValue = null, options = { useSession: false }) {
    const prefixedKey = this._getPrefixedKey(key);
   ...
   * @param {string} key - Storage key
   */
  removeItem(key: string, options = { useSession: false }) {
    const prefixedKey = this._getPrefixedKey(key);
    if (options.useSession) {
