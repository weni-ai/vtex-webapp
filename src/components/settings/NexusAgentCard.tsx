...
import { toggleNexusAgentActive } from '../../services/agent.service';
...
export function NexusAgentCard({ uuid, name, description, code, isActive, isOfficial }: NexusAgentCardProps) {
  ...
  const textColor = isActive ? '$fg-base' : '$fg-base-disabled';
  const descriptionColor = isActive ? '$fg-base-soft' : '$fg-base-disabled';

  const handleToggle = useCallback(async (checked: boolean) => {
    setIsToggling(true);
    try {
      await toggleNexusAgentActive(uuid, checked);
    } catch {
          ...
          <Flex align="center" justify="center" style={avatarInnerStyle}>
            <img
              ...
              style={{
                width: '40px',
                height: '40px',
                filter: isActive ? 'unset' : inactiveFilter,
