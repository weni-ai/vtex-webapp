import { Checkbox, Field, FieldDescription, Flex, Input, Label } from "@vtex/shoreline";
...
import { Feature, RootState } from "../../interfaces/Store";

export function TestContact({ code }: { code: Feature['code'] }) {
  const { formData = {}, setFormData } = useContext(SettingsContext) || {};

  const currentNumber = useSelector((state: RootState) => integratedAgents(state).find((agent) => agent.code === code)?.phone_numbers?.[0]);
  ...
  return (
    <Flex direction="column" gap="var(--sl-space-6)" style={{ marginBottom: 'var(--sl-space-6)' }}>
